...
### 0.5 插桩（仅 optimize 模式，**必需**）

> **铁律**：optimize 模式下必须先有阶段性能数据，才能讨论如何优化。没有阶段数据的优化讨论是凭直觉猜，禁止跳过此步骤。

在关键路径上**临时**添加计时打点，确保后续讨论有具体的性能归因依据。

**步骤**：
1. 识别目标代码的逻辑阶段——函数入口/出口、主要循环边界、I/O 边界、状态转换点等
2. 在每个阶段边界添加计时打点
3. 运行 benchmark，收集每阶段的 mean / median 耗时
4. 输出阶段性能表：

```
## 阶段性能数据（基线）

| 阶段 | 位置 | mean | median | 占比 |
|------|------|------|--------|------|
| stage 1: parse_input | parser.rs:42 | 12 µs | 11 µs | 18% |
| stage 2: validate    | parser.rs:87 | 8 µs  | 7 µs  | 12% |
| stage 3: transform   | parser.rs:120| 35 µs | 33 µs | 53% |
| stage 4: serialize   | parser.rs:180| 11 µs | 10 µs | 17% |
| **总计**             | —            | 66 µs | 61 µs | 100% |
```
...
- 插桩不应改变代码逻辑，仅做观测
- 占比之和应接近 100%，否则说明存在未被覆盖的阶段，需补充打点
- 阶段性能数据持久化到 `.artifacts/bench-stages-YYYYMMDD-HHMMSS.txt`

**进入 Phase 3 的前提**：阶段性能表已生成且占比覆盖完整。否则不得开始优化讨论。
...
> **前置要求**：本步骤必须基于 Phase 0.5 已收集的阶段性能数据（`bench-stages-*.txt`）展开。讨论中每个论点必须指向具体阶段的占比/耗时，禁止凭直觉提方案。
...
**讨论起点**：基于阶段性能表，识别占比最高的 1-2 个阶段作为优化目标。

**聚焦问题**：
- 方案针对的是哪个阶段？该阶段当前占比是多少？
- 方案是否真的能降低该阶段耗时？预期降低多少？
- 是否有更简单的替代方案能达到类似效果？
- 是否可能引入正确性问题或行为变化？
- 是否引入了不必要的复杂度？
