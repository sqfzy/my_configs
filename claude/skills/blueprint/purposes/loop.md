...
周期性重复执行 —— "跑一晚"、"自主演进"、"质量打磨"、"测试逼近"、"性能逼近"、"文档跟进" 等场景。loop 是**组合类元目的**：它不直接改代码，而是规划一份**循环契约**，契约里的循环体可以调任意 blueprint 子目的、`/report`、`/script`、shell 命令。审批后由 loop 执行层按契约反复执行，直到终止条件满足。

设计借鉴旧 `/repeat` skill 的执行层（两种模式、cooldown + 指数退避、subagent 返回协议、不暂停铁律），规划层由 blueprint 框架统一。
...
每轮执行的步骤序列，可含：
- 子 blueprint 调用（`/blueprint --<p> --auto`）
...
```
BODY:
  step 1: /blueprint --review --auto (lens: {LENS})
          → tier_list
  step 2: pick = tier_list.tier1.first_unflagged()
          if none: signal UNTIL("tier1 empty")
  step 3: kind = classify(pick)
          route kind → /blueprint --{feat|fix|reshape|upgrade|test|bench|doc} --auto
...
| `DOC_SYNC`   | watch commit → /blueprint --doc --auto | 用户打断 |

用户 `/blueprint --loop EVOLVE "聚焦生产级"` 即可套模板；也可给完全自由描述让 AI 合成 BODY。
...
- **BODY 调 `/blueprint --<子目的>` 必须显式 `--auto`**（否则每轮都卡 ExitPlanMode）
- **BODY 调 `/report` 必须显式 `--auto`**（否则每轮都要等草稿确认）
- **BODY 不允许调 `/blueprint --loop`**（禁递归）
