...
> **Agent 是封装上下文的盒子，不是 prompt 的快捷方式。**

创建 agent 的价值在于**上下文隔离** + **可复用的专业人格**——把一类任务的判断标准、工具边界、输出格式全部冻结到一个可独立召唤的子进程里。如果只是想省几行 prompt，直接写 prompt 更好。当且仅当需要让 Claude "暂时变成另一个人"（不同的专业视角、不同的输出协议、不同的工具白名单）时，才值得做成 agent。好的 agent 描述里包含**触发场景**和**不适用场景**，让主 Claude 能准确判定何时委托。
