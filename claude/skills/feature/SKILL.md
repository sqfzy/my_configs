...
从预定义角色库中选出存在**真实价值冲突**的角色组合，并说明选择理由和预期冲突点。
若预定义角色无法覆盖该功能的特殊领域，可临时创建自定义角色替换。

**预定义角色库**：

| 代号 | 角色         | 核心偏好                           | 适用场景                     |
|------|--------------|------------------------------------|------------------------------|
| R1   | 风险卫士     | 失败模式、边界条件、防御性编程     | 系统设计、并发、I/O           |
| R2   | 极简主义者   | 反对一切不必要的复杂性             | API 设计、抽象层级            |
| R3   | 性能狂热者   | 吞吐量 / 延迟 / 资源效率           | 热路径、数据结构选择          |
...
例如：实现游戏物理引擎时可自定义"数值稳定性专家（关注浮点精度和模拟发散）"；
实现编译器 pass 时可自定义"IR 设计者（关注中间表示的正交性和可组合性）"。

**选角规则**：三个角色必须在核心价值观上存在真实冲突，不能选立场相近的组合。
