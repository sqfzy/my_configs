...
- **`shared/production-grade.md`** — Mandate that code-modifying skills produce **production-grade code**, not just "works on happy path" demos. Covers correctness/boundaries, concurrency & resource safety, observability (leveled structured logging), security (untrusted input, injection, secrets hygiene), performance awareness, maintainability, tests, docs, and language-specific rules (Rust/C++/Python). Defines the 4-question self-check: ship-tomorrow?, maintainer-in-6-months?, fuzzer-proof?, oncall-at-3am-can-diagnose?. Wired into `/design`, `/improve`, `/refactor`, `/fix`, `/cleanup`, `/migrate`, `/evolve`, `/ship`, `/merge`.
