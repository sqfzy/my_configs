...
- `[related: <path1,path2,...>]`：关联计划路径——生成计划时读取这些 plan 作为约束和上下文。未指定则**自动搜索** `.artifacts/` 中所有 `plan-*.md`
- `[auto]`：无人值守模式——AI 严格按推荐哲学选择每个决策点的最优方案（追求设计最优，一步到位），直接生成完整 plan，用户事后审阅。禁止因"保守"或"兼容现有"而降级推荐

### 文件命名

plan 文件遵守产物统一存储约定：`plan-<项目名>-YYYYMMDD-HHMMSS.md`，存入 `.artifacts/`。如 `plan-auth-module-20260402-143000.md`。活跃 plan 通过文件内 `状态：` 字段检索（非"已完成"即活跃）。
...
2. **搜索关联计划**：自动搜索 `.artifacts/` 中所有 `plan-*.md`，加上 `[related]` 显式指定的路径。读取所有**活跃的**（状态非"已完成"）关联计划，提取已确定的决策作为约束和上下文
...
**`auto` 模式**：不逐维度询问，AI 严格按推荐哲学为每个决策点选择**设计最优**的方案，直接生成完整 plan.md，最后输出给用户审阅。auto 模式下禁止以下行为：
...
```bash
find .artifacts -name "plan-*.md" 2>/dev/null
```

所有搜索到的**活跃的** plan（状态非"已完成"）以及 `[related]` 显式指定的，都作为当前计划的**上下文和约束**。
...
```
.artifacts/
  plan-myproject-20260401-100000.md       ← 项目级（技术选型、架构、全局规范）状态：实施中
  plan-auth-module-20260402-143000.md     ← 模块级（接口细化、内部设计）状态：已确认
  plan-parser-20260403-091500.md          ← 模块级  状态：草案
  plan-myold-project-20260301-080000.md   ← 状态：已完成
```
