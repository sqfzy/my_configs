...
- **描述**（必填）：要计划的项目或功能
- `[target: <path>]`：plan.md 的输出路径，默认为项目根目录
- `[related: <path1,path2,...>]`：关联计划路径——生成计划时读取这些 plan 作为约束和上下文。未指定则**自动搜索**项目中所有 `plan.md` 和 `*.plan.md`
- `[auto]`：无人值守模式——AI 自行选推荐项，直接生成完整 plan.md，用户事后审阅
...
2. **搜索关联计划**：自动搜索项目中所有 `plan.md` 和 `*.plan.md`，加上 `[related]` 显式指定的路径。读取所有关联计划，提取已确定的决策作为约束和上下文
...
```bash
find . -name "plan.md" -o -name "*.plan.md" 2>/dev/null | grep -v node_modules | grep -v target | grep -v .git
```

所有搜索到的 plan（以及 `[related]` 显式指定的）都作为当前计划的**上下文和约束**。
...
```
plan.md                    ← 项目级（技术选型、架构、全局规范）
plans/
  auth.plan.md             ← 模块级（接口细化、内部设计）
  parser.plan.md
  storage.plan.md
```
