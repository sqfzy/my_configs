...
> **核心机制**：每轮通过 Agent tool 派发 subagent 执行 prompt，subagent 拥有独立上下文，执行完毕后返回一行摘要。主循环只维护轮次、冷却、进度等极简状态，上下文不会随轮次增长。
...
loop:
    ...
    2. 派发 subagent 执行 prompt（见下方 subagent 派发规则）
    3. 记录执行后状态：git rev-parse HEAD, git diff --stat
    4. 主循环检测进度（对比前后 git 状态）
    5. 主循环检查结束条件
       → 任一条件满足？break
...
每轮通过 Agent tool 派发 subagent，prompt 格式：

```
你在 <当前目录> 下工作。
执行以下任务（auto 模式，不暂停等待确认）：

<用户原始 prompt>

完成后用一行摘要总结本轮改动（格式："摘要：<内容>"）。若无改动则输出"摘要：无改动"。
```

**派发要求**：
- 使用 Agent tool 的 `description` 字段标注轮次：`"repeat 第 N 轮"`
- subagent 拥有独立上下文，不继承主循环的历史
- subagent 返回后，主循环仅提取摘要行用于状态输出，其余内容不保留
...
2. 通过 Agent tool 派发 subagent 执行用户的 prompt（按 subagent 派发规则）
...
4. 检查自定义终止条件（若有）——运行对应的检查命令

5. 输出本轮状态行（含 subagent 返回的摘要）
...
- **上下文隔离**：每轮通过 Agent tool 派发 subagent 执行，subagent 拥有独立上下文。主循环只保留每轮的一行摘要和进度状态，不累积 skill 执行的详细输出。这意味着：
  - 主循环的上下文不会随轮次增长，可以长时间运行
  - 每轮 subagent 都有完整的上下文窗口可用，不受前序轮次影响
  - subagent 之间无共享状态——轮次间的连续性通过代码变更（git）和产物（.artifacts/）传递
