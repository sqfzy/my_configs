...
0. **重新输出执行参数**（防止上下文压缩后遗忘原始命令）：
   ```
   [repeat] 命令：<原始 prompt> | 结束条件：<条件> | 轮次：N | 冷却：<当前冷却>s
   ```
...
**上下文效率**：每轮执行后仅保留一行状态摘要在输出中。skill 执行过程中产生的详细分析、代码阅读结果等中间产物不需要在 repeat 的循环上下文中保留——它们的价值已经体现在代码改动和 .artifacts/ 报告中。
...
- **上下文管理**：长时间运行的循环会累积大量上下文。每轮执行的 skill 输出、git diff、状态检查都会占用上下文窗口。当上下文接近压缩阈值时，早期轮次的详细输出会被压缩，可能影响后续轮次的判断质量。建议：
  - 重量级 skill 组合（如 `/evolve auto implement`）建议 `max-rounds` 或适当的时间限制，避免单次会话跑太多轮
  - 若需要长时间持续运行，建议分多个会话执行，每个会话设定合理的轮数上限
  - 每轮的状态行输出保持简洁（一行摘要），避免在循环内输出大段分析文本
