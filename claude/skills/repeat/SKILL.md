...
║                                                      ║
║  ⏰ 终止条件 ： <until 条件>                         ║
║  ⚙️  执行模式 ： 直接执行 / subagent                 ║
║  🕐 当前时间 ： <HH:MM:SS>                           ║
║  ⏳ 已运行   ： <X 小时 Y 分>                        ║
║                                                      ║
║  📊 累计进度                                         ║
║     轮次：N  ·  有效：M  ·  无效：K                  ║
║     连续无效轮：P  ·  当前 cooldown：<30s/1m/…>      ║
║                                                      ║
╚══════════════════════════════════════════════════════╝
```
...
diff-stat 风格，标题行带状态图标：

```
── 第 N 轮 ✅ abc1234 · 耗时 2m18s ──
...
- **`/loop`**（Claude Code 内建）：按固定时间间隔触发；`/repeat` 则是按条件循环到目标达成。前者适合"每 5 分钟检查一次"，后者适合"跑到所有测试通过为止"
- **`/schedule`**（Claude Code 内建）：cron 远程触发。`/repeat` 是单次会话内的循环，`/schedule` 是跨会话的周期性任务
- **`/autopilot`**：单一长任务的无人值守看护（契约驱动 + 自愈）。`/repeat` 是重复执行**同一**动作，`/autopilot` 是监护**一个**长任务——语义截然不同
- **`/retro`**：循环结束后可用 `/retro` 做一次性复盘，提炼循环期间的共性教训
