...
| 关键词/意图 | 推断模式 |
|-------------|----------|
| "拆分"、"重命名"、"提取"、"内联"、"移动" | 默认模式 |
| "重新设计"、"重塑 API"、"替换核心抽象"、"架构重组" | breaking ⚠️ |
| "渐进式替换"、"逐步迁移"、"strangler"、"新旧共存" | breaking（自动检测渐进式） ⚠️ |

- 多个关键词匹配多个模式时，按合理顺序组合执行
- 无法推断时使用默认模式
- 推断结果输出一行声明：`▶ 推断模式：<mode>（从"<关键词>"推断）`
- 高风险模式（breaking）推断后需用户确认（auto 模式除外）

---

## 渐进式迁移（breaking 模式自动检测）

breaking 模式启动时，自动检查 `.artifacts/` 中是否存在 `refactor-tracker-*.md` 跟踪文件。

### 检测逻辑

```
检查 .artifacts/refactor-tracker-*.md：

有跟踪文件且状态为"迁移中" → 进入"继续迁移"模式：
  - 读取跟踪文件，显示迁移进度（已迁移 N/M 个调用方）
  - 跳过 Phase 1（分析）和 Phase 2（方案设计），直接进入 Phase 3 继续迁移
  - 本次迁移下一批调用方
  - 更新跟踪文件
  - 所有调用方迁移完 → 切换状态为"清理中"

有跟踪文件且状态为"清理中" → 进入"清理"阶段：
  - 搜索所有 deprecated 标注、兼容层、adapter、feature flag
  - 列出待清理项让用户确认（auto 模式直接清理）
  - 逐一删除旧代码，每删一项运行测试验证
  - 全部清理完 → 更新跟踪文件为"已完成"，最终提交

无跟踪文件 → 正常执行 Phase 0-1-2，分析影响范围：
  - 调用方 ≤ 10 → 一次性完成（现有行为）
  - 调用方 > 10 → 建议渐进式迁移，创建跟踪文件：
    ```
    ⚠️ 影响范围较大（N 个调用方），建议渐进式迁移。
    渐进式：每次迁移一批调用方，跨会话跟踪进度，最后统一清理旧代码。
    一次性：本次会话内完成所有迁移。
    [渐进式 / 一次性]
    ```
    auto 模式：调用方 > 10 自动选渐进式。
```

### 跟踪文件格式

创建渐进式迁移时，生成 `.artifacts/refactor-tracker-<name>.md`：
...
## 调用方迁移状态
| 调用方 | 文件:行号 | 状态 | 会话时间 | Commit |
|--------|-----------|------|----------|--------|
| <caller> | <file>:<line> | ⏳ 待迁移 | — | — |
| ... | ... | ... | ... | ... |

## 兼容层
| 位置 | 类型 | 清理条件 |
|------|------|----------|
| <file>:<line> | deprecated wrapper | 所有调用方迁移后删除 |
| ... | ... | ... |
...
每次会话更新跟踪文件，同时追加 `.artifacts/INDEX.md` 记录。

### 清理阶段

当所有调用方标记为"已迁移"后，下次运行自动进入清理阶段：

1. 搜索所有待清理项：
   - `#[deprecated]` / `@deprecated` / `// DEPRECATED` 标注
   - 兼容层/adapter/shim 模块（从跟踪文件的"兼容层"表获取）
   - feature flag（用于新旧切换的编译开关）
   - 旧接口的空壳/转发函数

2. 列出清理清单让用户逐项确认（`auto` 模式直接清理）

3. 逐一删除，每删一项运行测试验证通过后再继续

4. 全部清理完成后：
   - 更新跟踪文件状态为"已完成"
   - 最终全量测试 + benchmark 回归验证
   - 提交：`refactor!(<scope>): complete migration — remove deprecated <旧接口>`
