...
argument-hint: "[--decision|--status|--incident|--issue|--release|--retro|--experiment|--primer|--repro] [--audience dev|user|both] [<主题或补充素材>] [--auto]"
...
| `issue`      | 发现中 | 协作者 / 自己 backlog | 现象 + 复现 + 上下文 + **建议调查方向**（粗粒度） |
| `release`    | 已发布 | 终端用户 | 新能力 + break + 升级指引 |
| `retro`      | 事后 | 自己 / 团队 | 做对 / 做错 / 下次怎么改（反思性） |
| `experiment` | 验证后 | 自己 / 团队 | 假设 → 方法 → 数据 → 结论 |
| `primer`     | 静态快照 | onboard 中的开发者 / 终端用户（`--audience dev\|user\|both`） | 既有项目导览：结构 / 数据流 / 扩展点 / 陷阱；**纯描述，不评价不建议**（评价归 /pax --review，repo 内 doc 归 /pax --doc） |
| `repro`      | 即刻可执行 | 工程师 / SRE / 维护者 | 把现象 / 构建 / 数据封装成**可粘贴执行的配方**：环境锁定 + 编号步骤 + 预期产物 + 验证判据；与 issue 的差异 = 极精 vs 粗粒度，与 experiment 的差异 = 无统计推断 |
...
- **primer**：项目定位、目录结构、关键抽象与契约、数据流、build/test/run（dev）；或者定位、安装、典型用例、不支持场景（user）
- **repro**：复现目标、commit hash、工具链版本、输入与数据（含哈希 / 种子）、可粘贴执行的步骤序列、可观察的成功判据
...
**语言深浅**按 mode 不同：
- 对终端用户（release / primer --audience user） → 避免术语、多用例子
- 对经理（status） → 结论 + 进度 + 风险 + 量化
- 对工程师（decision / retro / experiment / issue / primer --audience dev / repro）→ 技术细节可以详
...
- primer：结构 + 数据流 + 抽象 + 陷阱（纯描述，不评价不建议）
...
**时区铁律**：所有时间戳（文件名 / INDEX.md / 报告头部 / 时间线）默认 **UTC+8（东八区，Asia/Shanghai）**——除非用户原话或 mode 骨架显式要求 UTC / 别的时区。涉及"现在几点"、"耗时起止"、"`Captured at`"、"`Reproduced`" 等都以东八区为准。获取时间用 `TZ=Asia/Shanghai date '+%Y-%m-%d %H:%M:%S'`（或等价方式）。

1. `mkdir -p .artifacts`
2. 写入 `.artifacts/<mode>-YYYYMMDD-HHMMSS.md`（时间戳为 report 生成开始时间，东八区）
   - **例外**：某些 mode 在特定条件下产出**目录而非单文件**（如 primer 对大项目拆成多份）。此时写入 `.artifacts/<mode>-YYYYMMDD-HHMMSS/` 目录，内含 `INDEX.md` 与多份子报告。mode 骨架文件定义何时走目录式 + 目录结构细节。
3. 更新 `.artifacts/INDEX.md`：
   - 若不存在，创建并写入表头
   - 追加一行数据；文件列填单文件名或目录名（目录名末尾 `/`）
...
**onboard 一个陌生项目**（不对应某个 pax 目的）：直接 `/report --primer [--audience dev|user|both]`，产物放 `.artifacts/primer-*.md` 作为私人心智模型快照。若之后想把它改造成 repo 内公开文档，再用 `/pax --doc` 基于此稿重写。
