...
记忆会腐烂。三个月后你只会记得"那段时间改了某个模块"，但记不住具体踩了哪些坑、为什么选了 A 而不是 B、哪个假设后来被推翻。`/retro` 的任务是在你还记得细节的当下，把**教训 + 上下文 + 取舍理由**固化下来——这样六个月后当你（或其他人）再次遇到相似问题时，不必重新踩一遍坑。复盘没有上下文就只是情绪宣泄，有上下文才是可检索的工程资产。
