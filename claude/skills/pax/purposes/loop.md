...
- 只有 UNTIL / BOUNDS / 用户打断才能**终止循环**。BATCH_UNTIL 只是**一批的边界**，不是循环的终止。
...
**subagent 模式批次交接铁律**（新增，关死 BATCH_DONE 假出口）：

- 主循环收到 **BATCH_DONE** → **必须立即派发下一个 subagent**，透传完整 STATE。**不打印总汇报、不给"后续建议"、不列 tier 预告、不问用户**。
- 主循环收到 **CONTINUE**（context 耗尽） → 同上，派发新 subagent 继承 STATE
- 主循环收到 **DONE** → UNTIL 已满足，**才**打印总汇报 + 可选 retro
- 任何把 BATCH_DONE 当成"交还给用户"或"等用户说继续"的行为 = **违反铁律**，等同提前终止
- auto 模式下不存在"用户会点继续"这种假设。唯一外部干预是用户主动打断。
...
| **主循环收 BATCH_DONE 后停下给"后续建议" / 等用户说继续** | **拒绝**——BATCH_DONE ≠ 循环终止，必须立即派发下一批。auto 下无"用户点继续"这种假设 |
