...
4. **时区默认 UTC+8（东八区，Asia/Shanghai）**——除非用户原话里显式带时区（"UTC 9点"、"美东时间"）。所有 UNTIL 里的时间、轮头部"当前时间"、轮间冷却"下次执行时间"、终止汇报里的"总耗时起止"全部以东八区为准。涉及时间比较时用 `TZ=Asia/Shanghai date` 或等价方式获取当前时间。
